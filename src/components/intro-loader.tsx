...
const DrawingSpring = ({ show }: { show: boolean }) => {
  ...
  useEffect(() => {
    ...
    // timings
    const drawDuration = 2500; // ms total to draw all
    const pause = 500; // ms pause when fully drawn
    const eraseDuration = 2500; // ms total to erase all

    // schedule draw animations proportional to length
    let cum = 0;
    paths.forEach((p, i) => {
      const L = lengths[i];
      const start = (cum / total) * drawDuration;
      const dur = Math.max(100, (L / total) * drawDuration); // min duration safeguard
      const a = p.animate(
    ...
    // schedule erase animations (after drawDuration + pause), same proportional scheme
    paths.forEach((p, i) => {
      const L = lengths[i];
      // compute cumulative before this path
    ...
    // cleanup on unmount / show change
    return () => {
      animsRef.current.forEach((a) => a.cancel());
      animsRef.current = [];
    };
  ...
  return (
    <div
      className={cn(
        "relative w-48 h-48 flex items-center justify-center transition-opacity duration-700",
      ...
      >
        {PATHS.map((d, i) => (
          <path
            ...
            fill="none"
            stroke="white"
            strokeWidth={50} /* ajusta según quieras grosor */
            strokeLinecap="round"
...
export default function IntroLoader({ onFinished }: IntroLoaderProps) {
  ...
  useEffect(() => {
    const timers = [
      ...
      window.setTimeout(() => setPhase(2), 5500), // show logo (2500+500+2500 = 5500)
